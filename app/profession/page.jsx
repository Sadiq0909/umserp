  ...
  return (
    <div className="min-h-screen p-6 text-gray-900 dark:text-gray-100">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8 justify-center">
        <Input
          placeholder="Enter a profession (e.g., Data Scientist)"
          value={profession}
      ...
      {result && (
        <div className="flex flex-col gap-8">
          ...
          <Card className="border border-gray-200 dark:border-gray-700 shadow-lg hover:shadow-xl transition p-6">
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {result.aboutProfession.careerPath.map((role, i) => (
                    ...
                    >
                      <CardHeader>
                        <CardTitle className="text-lg font-bold">{role.level}</CardTitle>
                      </CardHeader>
                      <CardContent>
              ...
              <Card className="border border-gray-200 dark:border-gray-700 shadow-sm p-4 mt-6">
                <CardHeader>
                  <CardTitle className="text-lg font-bold">Industry Demand</CardTitle>
                </CardHeader>
                <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {result.roadmap.map((phase, i) => (
              <Card key={i} className="border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg transition">
                <CardHeader>
        ...
        <p className="text-3xl font-bold">Learning Resources</p>
          ...
          <Accordion type="multiple" className="space-y-2">
                ...
                <AccordionContent>
                  <ul className="list-disc list-inside space-y-1">
                    {Object.entries(result.learningResources.onlineCourses).map(
                      ([platform, courses], i) => (
                        <li key={i}>
                          <span className="font-medium">{platform}:</span> {courses}
                        </li>
                      )
                    )}
                  </ul>
                ...
                <AccordionContent>
                  <ul className="list-disc list-inside space-y-1">
                    {Object.entries(result.learningResources.books).map(
                      ([category, books], i) => (
                        <li key={i}>
                          <span className="font-medium">{category}:</span> {books}
                        </li>
                      )
                    )}
                  </ul>
            ...
            {result.learningResources.datasetsAndProjects && (
              <AccordionItem value="datasetsAndProjects">
                ...
                <AccordionContent>
                  <ul className="list-disc list-inside space-y-1">
                    {Object.entries(result.learningResources.datasetsAndProjects).map(
                      ([type, items], i) => (
                        <li key={i}>
                          <span className="font-medium">{type}:</span> {items}
                        </li>
          ...
          <Card className="border border-gray-200 dark:border-gray-700 shadow-sm p-4 hover:shadow-lg transition">
            <CardHeader>
              <CardTitle className="text-3xl font-bold">Week by Week Action Plan</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="mb-4">{result.weeklyPlan.description}</p>
              <div className="grid md:grid-cols-2 gap-4">
                {result.weeklyPlan.schedule.map((weekStr, i) => (
                  <Card key={i} className="shadow-sm hover:shadow-md transition p-4 border border-gray-200 dark:border-gray-700">
                    <CardHeader>
                      <CardTitle className="text-lg">
                        <span className="font-semibold">{weekStr.split(":")[0]}:</span>{" "}
                        {weekStr.split(":")[1]}
                      </CardTitle>
                    </CardHeader>
                  </Card>
          ...
          {result.closingStatement && (
            <p className="text-center text-lg mt-6">{result.closingStatement}</p>
          )}
        </div>
      )}
    </div>
